...
int main(int argc, char **argv) {
	...
	auto findMatchesAndTriangulation = [&](const SRef<Keyframe> & newKf, const std::vector<uint32_t> &idxBestNeighborKfs, std::vector<SRef<CloudPoint>> &cloudPoint) {
		...
		for (int i = 0; i < idxBestNeighborKfs.size(); ++i) {
            ...
            float distPose = std::sqrt(std::pow(newKf_pose(0, 3) - tmpKf_pose(0, 3), 2.f) + std::pow(newKf_pose(1, 3) - tmpKf_pose(1, 3), 2.f) +
                std::pow(newKf_pose(2, 3) - tmpKf_pose(2, 3), 2.f));
			if (distPose < 0.05)
				continue;
	...
	auto processNewKeyframe = [&](SRef<Frame> &newFrame) -> SRef<Keyframe> {
		...
		LOG_INFO("Number of new 3D points before fusing: {}", newCloudPoint.size());
		...
		LOG_INFO("Keyframe: {} -> Number of new 3D points: {}", newKeyframe->getId(), newCloudPoint.size());
	...
	while (true)
	{
		...
		LOG_INFO("Nb of Local Map / World Map: {} / {}", localMap.size(), pointCloudManager->getNbPoints());
		LOG_INFO("Index of current reference keyframe: {}", referenceKeyframe->getId());
