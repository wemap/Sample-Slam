...
void init(std::string configFile)
{
    ...
    for (int k = 0; k < static_views_no; ++k)
    {
        ...
        std::cout << "display k"<<k << "     ->img size: " << static_views[k]->getWidth() << " " << static_views[k]->getHeight() << std::endl;
        viewer->display("__view", static_views[k]);
    }
    ...
    std::cout<<"<loading static view: ";
       ...
       static_views.resize(2);

       std::stringstream buff0, buff1;

       buff0<<std::setfill('0')<<std::setw(5)<<3;

       std::string path_temp0 = streamSource +   buff0.str() + ".png";
       buff1<<std::setfill('0')<<std::setw(5)<<4;

       std::string path_temp1 = streamSource +   buff1.str() + ".png";

 

       std::cout<<" view 0: "<<path_temp0<<std::endl;

       std::cout<<" view 1: "<<path_temp1<<std::endl;


       imageLoader->loadImage(path_temp0,static_views[0]);

       imageLoader->loadImage(path_temp1,static_views[1]);

       viewer->display("__view 0",static_views[0]);

       viewer->display("__view 1 ",static_views[1]);
...
bool debug_coplanarity()
{
    ...
    std::ofstream oxx("solar_cloud_temp.txt");
...
bool addFrameToMapAsKeyFrame(SRef<Frame> &frame, SRef<Image> &view, int newIndex)
{
    ...
    std::ofstream oxx("new_points.txt");
...
bool init_mapping(SRef<Image> &view_1, SRef<Image> &view_2, const std::string &path_cloud = std::string())
{
    ...
    if (!debug_fk)
    {
        ...
        myfile1.open("kp1_study.txt");

        for (unsigned int k = 0; k < kp1.size(); k++)
        {
            //std::cout<<" X,Y 1 : "<<  kp1[k]->getX()<<" "<< kp1[k]->getY()<< std::endl;
            myfile1 << " X,Y 1 : " << kp1[k]->getX() << " " << kp1[k]->getY() << "\n";
        }

        myfile1.close();

        std::ofstream myfile2;
        myfile2.open("kp2_study.txt");

        for (unsigned int k = 0; k < kp2.size(); k++)
        {
            //std::cout << " X,Y 2 : " << kp2[k]->getX() << " " << kp2[k]->getY() << std::endl;
            myfile2 << " X,Y 1 : " << kp2[k]->getX() << " " << kp2[k]->getY() << "\n";
        }
        myfile2.close();
        debug_fk =true;
    }
    ...
    for (unsigned int k = 0; k < matchedKeypoints1.size(); k++)
    {
       // std::cout<<" X,Y 1 : "<<  matchedKeypoints1[k]->getX()<<" "<< matchedKeypoints1[k]->getY();
      //  std::cout<<" X,Y 2 : "<<  matchedKeypoints2[k]->getX()<<" "<< matchedKeypoints2[k]->getY()<<std::endl;
    ...
    for (unsigned int k = 0; k < ggmatchedKeypoints1.size(); k++)
    {
       // std::cout<<" X,Y "<<  ggmatchedKeypoints1[k]->getX()<<" "<< ggmatchedKeypoints1[k]->getY()<<std::endl;
...
void idle_static()
{
    ...
    if(triangulation_first){
        ...
        if(init_mapping(/*static_views[3],static_views[4], path_cloud*/ static_views[0],static_views[1], path_cloud)){
    ...
    if (triangulation_first)
    {
        std::string path_cloud = "old_points.txt";
...
std::vector<SRef<Point2Df>> debug_pt1;
std::vector<SRef<Point2Df>> debug_pt2;
...
int main(int argc, char *argv[])
{
    ...
    ///--- debug test to check fundamental matrix
#ifdef CROSS_CHECK_FUNDAMENTAL_MATRIX
    std::ifstream file_pt1("pt1_log.txt");
    ...
    for (unsigned int c = 0; c < nb1; c++)
    {

        file_pt1 >> x_c >> y_c;
        //std::cout<<x_c<<" "<<y_c<<std::endl;
        debug_pt1.push_back(xpcf::utils::make_shared<Point2Df>(x_c, y_c));
    }

    std::ifstream file_pt2("pt2_log.txt");
    int nb2;
    file_pt2 >> nb2;

    for (unsigned int c = 0; c < nb2; c++)
    {

        file_pt2 >> x_c >> y_c;
        debug_pt2.push_back(xpcf::utils::make_shared<Point2Df>(x_c, y_c));

        // std::cout<<x_c<<" "<<y_c<<std::endl;
    }

    SRef<solver::pose::I2DTransformFinder> fundamentalFinder_debug = xpcf::ComponentFactory::createInstance<SolARFundamentalMatrixEstimationOpencv>()->bindTo<solver::pose::I2DTransformFinder>();

    fundamentalFinder_debug->find(debug_pt1, debug_pt2, F);
    std::cout << " Nb elements :" << debug_pt1.size() << " " << debug_pt2.size() << std::endl;

    std::cout << "DEBUG MATRIX FUND" << std::endl;
