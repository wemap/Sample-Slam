...
bool tracking(SRef<Image>&view){
   ...
   /*
    ...
    if(PnP->estimate(pt2d,pt3d,imagePoints_inliers, worldPoints_inliers, relative_pose) == FrameworkReturnCode::_SUCCESS /*&&
       ...
       // std::cout<<" pnp inliers size: "<<worldPoints_inliers.size()<<" / "<<pt3d.size()<<std::endl;
        ...
        DrawPointCloud(relative_pose);
        DrawPnpMatches(pt2d,pt3d,relative_pose);
